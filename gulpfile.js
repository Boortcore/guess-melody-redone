...
gulp.task('timer', function () {
  return gulp.src('js/timer.js')
    .pipe(plumber())
    .pipe(gulp.dest('build/js'));
});

gulp.task('animate', function () {
  return gulp.src('js/animate.js')
    .pipe(plumber())
    .pipe(gulp.dest('build/js'));
});
...
gulp.task('copy', ['copy-html','player', 'animate', 'music', 'timer', 'scripts', 'style'], function () {
  return gulp.src([
    'fonts/**/*.{woff,woff2}',
    'img/*.*'
  ], {base: '.'})
...
gulp.task('build', ['assemble', 'imagemin', 'player', 'animate', 'music']);
